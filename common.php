...
{
    $cache_file_path = 'cache/public-key.pem';
    $log_file_path = 'cache/public-key.log';
    if (file_exists($cache_file_path)) {
        ...
        file_put_contents($log_file_path, 'public key retrieved at ' . date('c'), FILE_APPEND);
    }
    try {
        ...
        if ($cached_key === $public_key) {
            ...
            file_put_contents($log_file_path, 'public key updated at ' . date('c'), FILE_APPEND);
        }
        $jwt_payload = JWT::decode($jwt, $cached_key, ['ES256']);
        return $jwt_payload;
    }
